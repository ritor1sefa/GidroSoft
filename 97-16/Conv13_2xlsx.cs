...
namespace MaxyGames.Generated {
	public class b_Conv13_2xlsx : MaxyGames.RuntimeBehaviour {
		...
		private void Start() {
			...
			Files.Clear();
			Tables = sql_master_tables("_emptyY");
			foreach(string loopObject in Directory.GetFiles(Application.streamingAssetsPath + "/" + "files/bd/13/", "y_*.sqlite")) {
		...
		private bool sql_connect(string db_name) {
			...
			path = Application.streamingAssetsPath + "/" + "files/bd/13/" + db_name + ".sqlite";
		...
		public System.Collections.IEnumerator xlsx_save(List<List<string>> finalTable) {
			...
			for(int index3 = 0; index3 < finalTable.Count; index3 += 1) {
				...
				for(int index4 = 0; index4 < row_list.Count; index4 += 1) {
					...
					if((clmn_int == 1)) {
						//год
						wSh.Cell(row_int, clmn_int).Value = raw_value.Remove(0, 2);
					} else {
						raw_value = new Regex("[@]").Replace(raw_value.Replace("  ", "_").Replace(" ", "_"), "");
						if((clmn_int == 2)) {
							//првоерка на дубликатность
			...
			wb.SaveAs(UnityEngine.Device.Application.streamingAssetsPath + "/files/xlsx/" + "13/" + currentTable + ".xlsx");
			row_int = 0;
